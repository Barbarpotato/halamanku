...
const VerticalStepper = ({ totalPages, contentNumber, contentId }) => {
		...
		const fetchData = async () => {
			...
			try {
				const [reactionsRes, questionsRes] = await Promise.all([
					fetch(`/api/ebook-user-content-reaction?contentId=${contentId}`, {
						headers: { Authorization: `Bearer ${tokenRef.current}` },
					}),
					fetch(`/api/ebook-user-content-question?contentId=${contentId}`, {
						headers: { Authorization: `Bearer ${tokenRef.current}` },
					}),
				]);
	...
	const refreshData = async () => {
		router.refresh();
		try {
			const res = await fetch(`/api/ebook-user-content-question?contentId=${contentId}`, {
				headers: { Authorization: `Bearer ${tokenRef.current}` },
			});
			if (res.ok) {
	...
	const handleSubmitQuestion = async (e) => {
		...
		try {
			...
			modal.show({ message: `Gagal ${modalMode === "create" ? "menambahkan" : "mengupdate"} pertanyaan` });
		} finally {
	...
	const handleDeleteQuestion = async (questionId) => {
		const confirmed = await modal.confirm({ message: "Apakah Anda yakin ingin menghapus pertanyaan ini?" });
		if (!confirmed) return;
	...
	const getPageStats = (pageNum) => {
		...
		const answerCount = data.reactions.filter((r) => r.react_type === "ANSWER").length;
		const thoughtCount = data.reactions.filter((r) => r.react_type === "THOUGHT").length;

		const emotionCounts = {
			like: data.reactions.filter((r) => r.emotion_type === "like").length,
			love: data.reactions.filter((r) => r.emotion_type === "love").length,
			thinking: data.reactions.filter((r) => r.emotion_type === "thinking").length,
			seen: data.reactions.filter((r) => r.emotion_type === "seen").length,
			dislike: data.reactions.filter((r) => r.emotion_type === "dislike").length,
			confused: data.reactions.filter((r) => r.emotion_type === "confused").length,
		};

		return { answerCount, thoughtCount, emotionCounts, questions: data.questions };
	};

	if (loadingData) {
		return (
			<div className="flex items-center justify-center py-12">
				<div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{ borderBottomColor: 'var(--color-primary)' }} />
				<p className="ml-3 text-gray-600">Memuat data interaksi...</p>
			</div>
		);
	}
		...
		<div className="w-full  mx-auto px-4">
			...
			<div className="relative pl-6 sm:pl-12">
				...
				{Array.from({ length: totalPages }, (_, i) => {
					...
					const { answerCount, thoughtCount, emotionCounts, questions } = getPageStats(pageNum);
					const hasQuestion = questions.length > 0;

					return (
						<div id={`page-${pageNum}`} key={pageNum} className="relative mb-10 last:mb-0">
							{/* Step Circle */}
							<div className="absolute -left-2 sm:-left-3 w-8 h-8 sm:w-12 sm:h-12 rounded-2xl bg-white border-4 flex items-center justify-center shadow z-10"
								style={{ borderColor: 'var(--color-primary)' }}>
								<span className="font-bold text-lg sm:text-2xl" style={{ color: 'var(--color-primary)' }}>{pageNum}</span>
							</div>
								...
								>
									<div className="flex items-center gap-4">
										<p className="text-sm text-gray-500">
											{questions.length} pertanyaan • {thoughtCount + answerCount} interaksi
										</p>
									</div>
									...
									<div className="flex items-center gap-4">
										...
										{isExpanded ? <FaChevronUp size={22} /> : <FaChevronDown size={22} />}
									</div>
								</div>
								...
								>
									<div className="px-4 sm:px-7 pb-8 border-t border-gray-100">
										...
										<div className="grid grid-cols-3 sm:grid-cols-6 gap-4 py-6 border-b">
											{[
												{ icon: FaThumbsUp, count: emotionCounts.like, color: "text-green-500" },
												{ icon: FaHeart, count: emotionCounts.love, color: "text-red-500" },
												{ icon: FaRegLightbulb, count: thoughtCount, color: "text-yellow-500" },
												{ icon: FaEye, count: emotionCounts.seen, color: "text-gray-500" },
												{ icon: FaQuestion, count: emotionCounts.thinking, color: "text-purple-500" },
												{ icon: FaThumbsDown, count: answerCount, color: "", style: { color: 'var(--color-primary)' } },
											].map((item, idx) => (
												<div key={idx} className="text-center">
													<item.icon className={`${item.color} text-2xl mx-auto mb-1`} style={item.style} />
										...
										<div className="py-6 border-b border-gray-100">
											<div className="font-semibold text-gray-900 mb-4">Overview</div>
											<div className="grid grid-cols-2 gap-6 text-sm">
												<div className="flex justify-between">
													<span className="text-gray-600">Total Thoughts</span>
													<span className="font-medium">{thoughtCount}</span>
												</div>
												<div className="flex justify-between">
										...
										{/* Action Section */}
										<div className="pt-6">
											<div className="font-semibold text-gray-900 mb-4">Action</div>
			...
			{isModalOpen && (
				...
				>
					...
					>
						...
						<h2 className="text-2xl sm:text-3xl font-bold mb-6 text-gray-900">Halaman {selectedPage}</h2>
						{pageImageUrl ? (
							<img
								src={pageImageUrl}
							...
							<p className="text-red-500 text-center py-12">Gagal memuat gambar halaman</p>
						)}
					</div>
				</div>
			)}
			...
			{showQuestionModal && (
				<div className="fixed inset-0 z-50 flex items-center justify-center px-4">
					<div className="fixed inset-0 bg-black/60" onClick={closeQuestionModal} />
					<div className="relative bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl">
						<div className="p-8">
							<h3 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-6">
								{modalMode === "create" ? "Tambah Pertanyaan" : "Edit Pertanyaan"}
							</h3>
							<textarea
						...
						<div className="bg-gray-50 px-8 py-6 flex gap-3">
							<button
								...
								className="flex-1 py-4 bg-white border border-gray-300 rounded-2xl font-medium text-gray-700 hover:bg-gray-100 transition-colors"
							>
								Batal
							</button>
							<button
								onClick={handleSubmitQuestion}
								disabled={creatingQuestion || !questionFormData.question.trim()}
								className="flex-1 py-4 disabled:opacity-50 text-white rounded-2xl font-semibold transition-colors"
								style={{ backgroundColor: 'var(--color-primary)' }}
							>
								{creatingQuestion ? "Menyimpan..." : modalMode === "create" ? "Tambahkan" : "Update"}
							</button>
						</div>
					</div>
				</div>
			...
			{loadingPage && (
				<div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60]">
					<div className="bg-white px-8 py-6 rounded-2xl shadow-xl flex items-center gap-4">
						<div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{ borderBottomColor: 'var(--color-primary)' }} />
						<p className="text-base sm:text-lg text-gray-700">Memuat halaman...</p>
					</div>
				</div>
			)}
		</div>
